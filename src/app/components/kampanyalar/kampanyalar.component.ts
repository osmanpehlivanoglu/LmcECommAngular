...
export class KampanyalarComponent implements OnInit {
  ...
  getUrunler() {
    this.urunlerKampanyaliSon = [];
    this.urunService.getAll().subscribe((response) => {
      this.urunler = response.data;

      this.urunler.forEach((element) => {
        if (this.kampanyaliUrunIdleri.includes(element.urunId)) {
          this.urunlerKampanyaliSon.push(element);
        }
      });
    });
  }
  ...
  getTarihArtan() {
    this.urunlerKampanyaliSon.sort(
  ...
  getFiyatArtan() {
    this.urunlerKampanyaliSon.sort((a, b) => a.bayiFiyati - b.bayiFiyati);
  }

  getFiyatAzalan() {
    this.urunlerKampanyaliSon.sort((a, b) => b.bayiFiyati - a.bayiFiyati);
  }
