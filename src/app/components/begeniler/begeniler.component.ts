...
@Component({
  selector: 'app-begeniler',
  templateUrl: './begeniler.component.html',
  styleUrls: ['./begeniler.component.css'],
})
export class BegenilerComponent implements OnInit {
  ...
  getBegenilerByKullaniciId(kullaniciId: number) {
    ...
    this.begeniIdleri = [];
    this.begeniService
      .getBegenilerByKullaniciId(kullaniciId)
      .subscribe((response) => {
  ...
  getUrunler() {
    this.urunlerBegenilenSon = [];
    this.urunService.getAll().subscribe((response) => {
      this.urunler = response.data.reverse();

      this.urunler.forEach((element) => {
        if (this.kullaniciBegenileri.includes(element.urunId)) {
          this.urunlerBegenilenSon.push(element);
        }
      });
    });
  }
  ...
  getFiyatArtan() {
    this.urunlerBegenilenSon.sort((a, b) => a.bayiFiyati - b.bayiFiyati);
  }

  getFiyatAzalan() {
    this.urunlerBegenilenSon.sort((a, b) => b.bayiFiyati - a.bayiFiyati);
  }
